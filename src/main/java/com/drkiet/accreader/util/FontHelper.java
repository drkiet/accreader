...
public class FontHelper {
	public static String updateFont(String text, int fontSize, String font) {
		...
		for (int idx = 0; idx < HelpPanel.START_TAGS.length; idx++) {
			text = text.replaceAll(HelpPanel.START_TAGS[idx], startFont + HelpPanel.START_TAGS[idx]);
			text = text.replaceAll(HelpPanel.END_TAGS[idx], HelpPanel.END_TAGS[idx] + "</font>");
		}
